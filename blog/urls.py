...
router.register(
    r"articles/(?P<article_id>\d+)/comments", views.CommentView, basename="comment"
)

urlpatterns = [
    path("categories/", views.CategoryListView.as_view(), name="category_list"),
    path(
        "articles/<int:article_id>/contents/<str:content_type>/",
        views.ContentCreateUpdateDeleteView.as_view({"post": "create"}),
        name="article_content_create",
    ),
    path(
        "articles/<int:article_id>/contents/<str:content_type>/<int:object_id>/",
        views.ContentCreateUpdateDeleteView.as_view({"put": "update"}),
        name="article_content_update",
    ),
    path(
        "articles/<int:article_id>/contents/<int:content_id>/delete/",
        views.ContentCreateUpdateDeleteView.as_view({"delete": "destroy"}),
        name="article_content_delete",
    ),
    path(
        "articles/<int:pk>/like-unlike/",
